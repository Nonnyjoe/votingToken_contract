...
async function main() {
  ...
  const votingToken = await VotingToken.deploy(1000000000000, "NONSO INU", "NNS");
...
const createPool = await votingToken.createVotePool2("0x13B109506Ab1b120C82D0d342c5E64401a5B6381","0xBB9F947cB5b21292DE59EFB0b1e158e90859dddb", "0xfd182E53C17BD167ABa87592C5ef6414D25bb9B4");
let event = await createPool.wait();
//console.log(event);
let child = await event.events[1].args;
let voteId = child[0];
console.log(`VOTE POOL ID IS ${voteId}`);
const contenders = votingToken.DisplayContenders(voteId);
const contenders2 = await contenders;
console.log(await contenders2);
const vote = await votingToken.Vote(voteId, "0xfd182E53C17BD167ABa87592C5ef6414D25bb9B4", "0xBB9F947cB5b21292DE59EFB0b1e158e90859dddb", "0x13B109506Ab1b120C82D0d342c5E64401a5B6381");

// close voting pool
await votingToken.closeVotingPool(voteId);
const closePool = votingToken.closeVotingPool(voteId);
const win = await votingToken.DisplayWinner(voteId);
